...
impl<'a> TreeSearcher<'a> {
    ...
    fn new_full(
        ...
        let out = TreeSearcher {
            sets,
            scores,
            active: HashSet::from_iter(0..scores.dim().0),
        ...
        for p in 0..np {
            ...
            for bundle in self.sets[p].iter() {
                for row_idx in bundle.indexes.iter() {
                    if self.active.contains(row_idx) {
...
#[extendr]
fn rust_predict(r_tree_mat: Robj, x_robj: Robj) -> Vec<usize>{
    let x_mat = <ArrayView2<f64>>::from_robj(&x_robj)
        .unwrap()
        .to_owned()
        .map(|x| OrderedFloat(*x));

    let r_tree_mat = <ArrayView2<f64>>::from_robj(&r_tree_mat)
        .unwrap();

    let mut tree : Vec<RNode> = Vec::new();
    for node in r_tree_mat.axis_iter(Axis(0)) {
        if node[0] == -1.0 {
             tree.push(RNode{
                cut_axis : None,
                cut_point: None,
                left_child: None,
                right_child: None,
                action : Some(node[1] as usize)
            })
        } else{
            tree.push(RNode{
                cut_axis : Some(node[0] as usize),
                cut_point: Some(OrderedFloat(node[1])),
                left_child: Some(node[2] as usize),
                right_child: Some(node[3] as usize),
                action : None
            })
        }
    }
